...
#include "math.h"
// Такс, первое тут список, сейчас распишу ниже, что происходит:
...
void add_el(struct list1 *current, FILE *input) {
	...
	z->next = NULL; // последний элемент моего списка
	current->next = z; // делаем переустановку указателя
}

// добавляю элементы списка для выходных данных
...
// Вроде бы подробно написал, а, точно.
//Это работа Клименко Максима, не указал))).
